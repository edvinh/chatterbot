...
rulesApply r = try (transformationsApply "*" reflect r)
...
substitute wc (t:ts) ss
  ...
  | otherwise = t:(substitute wc ts ss)
...
transformationApply wc func list tuple = mmap (substitute wc (snd tuple)) (mmap func (match wc (fst tuple) list))
...
transformationsApply wc func (p:ps) list = orElse (transformationApply wc func list p) (transformationsApply wc func ps list) 
